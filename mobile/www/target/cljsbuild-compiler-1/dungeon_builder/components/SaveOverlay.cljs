...
(defn handle-save [name tile-state terrain-state loaded-map-name currentMaps]
...
(defn SaveOverlay [active tile-state terrain-state loaded-map-name currentMaps]
    (fn [active tile-state terrain-state loaded-map-name]
      (let [name (atom loaded-map-name)]
        [:div.saveOverlay {:class (if active "active" "")}
          ...
          [:button {:on-click #(handle-save @name tile-state terrain-state loaded-map-name currentMaps)} "Save"]])))